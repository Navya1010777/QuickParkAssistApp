...
/**
 * CustomRestTemplateService provides utility methods to make HTTP requests
 * using RestTemplate.
 * It includes methods for authentication (login/logout), generic GET/POST/PUT
 * requests,
...
public class CustomRestTemplateService {
    ...
    private static final String BASE_URL = "http://localhost:7212/api"; // Base API URL
    ...
    /**
     * Handles user login by sending authentication request and storing session
     * cookies.
     * 
    ...
    public <T> ResponseEntity<ResponseDTO<T>> post(
        ...
        try {
            System.out.println("Sending request to: " + BASE_URL + route);
            try {
    ...
    public <T> ResponseEntity<ResponseDTO<T>> postMultipart(
        ...
        try {
            return restTemplate.exchange(BASE_URL + route, HttpMethod.POST, requestEntity,
                    getTypeReference(responseType));
