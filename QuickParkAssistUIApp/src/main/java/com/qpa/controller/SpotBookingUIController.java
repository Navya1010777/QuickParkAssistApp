...
public String saveBooking(
    ...
    try {
        ...
        model.addAttribute("message", "✅ Booking successfully added!");
        return "redirect:/ui/booking/viewAll"; // Redirect to view all bookings on success
        ...
        if (errorMessage.contains("Spot is not available")) {
            model.addAttribute("error", "❌ Spot with ID " + slotId + " is not available. Please select another spot.");
        } else if (errorMessage.contains("Spot is already booked")) {
            model.addAttribute("error", "❌ Spot with ID " + slotId + " is already booked for the given time. Please choose a different time slot.");
        } else if (errorMessage.contains("Start date, start time, and end time must be provided")) {
            model.addAttribute("error", "❌ Start Date, Start Time, and End Time are required fields. Please fill them correctly.");
            ...
            model.addAttribute("error", "❌ No vehicle found with Registration Number: " + registrationNumber + ". Please check and try again.");
        } else if (errorMessage.contains("Spot with ID")) {
            model.addAttribute("error", "❌ Spot with ID " + slotId + " does not exist. Please enter a valid Spot ID.");
        } else {
...
public String updateBooking(
    ...
    try {
        ...
        if (booking != null) {
            ...
            // 🛑 First, check if the update conflicts with another booking **before applying the update**
            try {
